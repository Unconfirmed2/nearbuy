...
-- Add merchant_id to users table for hierarchy (if not exists)
DO $$ BEGIN
...
-- Add all_stores_access flag for super merchants (if not exists)
DO $$ BEGIN
...
DROP POLICY IF EXISTS "Merchants can manage their invitations" ON public.invitations;
...
DROP POLICY IF EXISTS "Merchants can manage store permissions" ON public.user_store_permissions;
...
DROP POLICY IF EXISTS "Users can view their own permissions" ON public.user_store_permissions;
...
DROP POLICY IF EXISTS "stores_select_accessible" ON stores;
...
CREATE INDEX IF NOT EXISTS idx_user_store_permissions_user_store ON user_store_permissions(user_id, store_id);
